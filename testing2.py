...
if image_features:
    ...
    for i, label in enumerate(kmeans.labels_):
        if i < len(cropped_images):  # Ensure index is within valid range
            image_filename = os.path.basename(cropped_images[i])
            cluster_dir = cluster_dirs[label]
            new_path = os.path.join(cluster_dir, image_filename)
            os.rename(cropped_images[i], new_path)
